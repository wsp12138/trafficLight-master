...
if __name__ == '__main__':
    ...
    dqn_agents = {ts: DQN(env.observation_space.shape[0], env.action_space.n, cfg, int(ts)) for ts in env.ts_ids}
    rewards = []
    delays = []
    train_tims = []
    for i in range(100):
        ...
        while not done['__all__']:
            ...
            actions = {ts: dqn_agents[ts].choose_action(state[ts], epsilon) for ts in dqn_agents.keys()}
        ...
        for k in range(400):
            ...
            for agent in dqn_agents.values():
